...
function test_equal_can()
    msg1 = CANMessage(0x12345, collect(1:7), true)
    msg2_frm = CANalyze.CANFrame(0x12345, collect(1:7); is_extended=true)
    msg2 = CANMessage(msg2_frm)
    msg3 = CANMessage(0x123, collect(1:8), false)
...
function test_message_can()
    CANMessage(0x12345, collect(1:9), true) # throws AssertionError
end


function test_convert_can()
    msg = CANMessage(0x12345, collect(1:6), true)
    frm = convert(CANalyze.CANFrame, msg)
...
function test_equal_fd()
    msg1 = CANFDMessage(0x12345, collect(1:64), true, false, false)
    msg2_frm = CANalyze.CANFdFrame(0x12345, collect(1:64); is_extended=true)
    msg2 = CANFDMessage(msg2_frm, false)
    msg3 = CANFDMessage(0x123, collect(1:24), false, false, false)
...
function test_message_fd_over()
    CANFDMessage(0x12345, collect(1:128), true, false, false) # AssertionError
end


function test_message_fd_invalid_dlc()
    CANFDMessage(0x12345, collect(1:29), true, false, false) # AssertionError
end


function test_convert_fd()
    msg = CANFDMessage(0x12345, collect(1:32), true, false, false)
    frm = convert(CANalyze.CANFdFrame, msg)
